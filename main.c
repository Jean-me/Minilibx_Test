...
int	main(void)
{
	...
	img.addr = mlx_get_data_addr(img.img, &img.bits_per_pixel, &img.line_length,
			&img.endian);
	// Draw a rectangle
	draw_rectangle(&img, 100, 300, 100, 400);
	...
	// //画像をウィンドウに表示
	draw_julia(&img, c_re, c_im);
	mlx_put_image_to_window(mlx.mlx, mlx.win, img.img, 0, 0);
	mlx_hook(mlx.win, KeyPress, KeyPressMask, event_handler, mlx.mlx);
	mlx_hook(mlx.win, DestroyNotify, StructureNotifyMask, close_window,
		mlx.mlx);
	//マウス座標表示
